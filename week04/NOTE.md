...
注1：
关于乘以16，以及”空出一位“的解释如下。

node获取响应消息的时候，body里面，都会先是长度，再是内容。length*16是把前一次循环读取的长度的字符的16进制转为10进制的数字。空出来实际就是比如十进制：原先1你乘以了10，就变成了10，那么1现在在十位上面了吧，再乘以10，变成100，1就跑到百位上面了，相当于把字符往前移动，那么后面就空出来了，之前说了，node里第一行是下一行对应内容的长度的16进制字符，假设是54，那么读取字符的时候，先读取的是5，5读取的时候，先this.length*16，因为初始化的时候length是0，所以length还是0，然后下一行length赋值给char（也就是54中的5的16进制对应的十进制整数），然后继续读取下一个字符，次数length=5，16*5，相当于把5提升到十位上面，后面留下个位给到54里面的4.差不多就是这个意思我个人理解下来。

你要知道在 receiveChar 处理字符是一位一位读进去的，在处理 length 的时候，同时还要把读进去的代表长度的字符转成数字。
比如处理表示长度的字符串 '1c'，根据它得到  this.length = 28
1. 一种办法把数字字符存在一个变量里，等读完一起用 Number('1c') 转。
2. 还有一种更简单的办法就是像老师讲的，读 '1' 的时候 this.length = 1，读 c 的时候知道前面已有的数字在高位上，把之前的数字移动到高位上，就是 *16。然后在加上新读进来的数字 c。
